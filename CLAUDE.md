...
- `gui.py` — customtkinter launcher; printer status queried in background threads (30s auto-refresh), 1,000-line log ring buffer, server-poll generation tokens; tabbed UI ([기본]=server+kiosk on one screen / [템플릿]) with server-independent template preview/test-print; template layout editor with live in-window preview (debounced compose.render at dpi 72)
...
- **Versioning**: `VERSION` in `app/__init__.py`, release notes in `app/changelog.py` (`CHANGELOG` list) — update both when releasing; mirror in the outer repo's `CHANGELOG.md`. Current: v2.7.0
